...
extension AsyncableCas {
    ...
    private func saveData(_ mutable: Mutable, _ data: Data?) async throws -> String? {
        ...
        let parent = await mutable.parent
        ...
        let newParent = Parent(commitId: commitId, blobId: blobId)
        
        await mutable.updateParent(newParent)
    ...
    public func loadData(_ mutable: Mutable) async throws -> Data? {
        guard let blobId = await mutable.parent?.blobId else {
            return nil
        }
    ...
    public func loadDeletedJSON<T: Decodable>(_ mutable: Mutable) async throws -> T? {
        guard let mutableParent = await mutable.parent, mutableParent.blobId == nil else {
            return nil
        }
